...
class Test_Flight_Model(object):
    ...
    @pytest.mark.parametrize('depart,arrive,expected',[
        (timezone.datetime(2018,4,22,17,00,00),timezone.datetime(2018,4,22,22,00,00),timezone.timedelta(hours=5)),
    ])
    def test_travel_time(self,depart,arrive,expected):
        assert Flight(depart_time=depart,arrive_time=arrive).travel_time() == expected

    def test_travel_time_localize(self,atl_airport,boi_airport):

        depart = atl_airport.get_tz_obj().localize(timezone.datetime(2018,5,11,6,0,0))
        arrive = boi_airport.get_tz_obj().localize(timezone.datetime(2018,5,11,13,50,0))
