...
class FlightForm(forms.ModelForm):
    class Meta:
        model=Flight
        fields = ('origin_airport','destination_airport','depart_time','arrive_time','wanna_get_away','anytime','business_select',)
class SearchForm(forms.Form):
    origin_airport = forms.ModelMultipleChoiceField(Airport.objects.filter(sw_airport=True),label='Origin Airports',widget=forms.CheckboxSelectMultiple)
    destination_airport = forms.ModelMultipleChoiceField(Airport.objects.filter(sw_airport=True),label='Destination Airports',widget=forms.CheckboxSelectMultiple)
    depart_date = forms.DateField(label='Departure Date',widget=forms.TextInput(attrs={'class':'datepicker'}))
