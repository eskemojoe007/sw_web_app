...
class SW_Sel_base(object):
    required_keys = ['departureDate',
                     'destinationAirportCode', 'originationAirportCode']
...
class SW_Sel_Card(SW_Sel_base):
    ...
    def save_all_flights(self, **kwargs):
        for case in self.cases:
            SW_Sel_Single(browser=self.browser, search=self.search,
...
class SW_Sel_Case(SW_Sel_base):
    ...
    def get_result_soup(self, url=None, browser=None, timeout=10):
        ...
        try:
            ...
            wait.until(EC.element_to_be_clickable(
                (By.CLASS_NAME, "currency_dollars")))
    ...
    def save_all_flights(self, **kwargs):
        ...
        outbounds = outbound_table.findAll(
            'li', attrs={'class': 'air-booking-select-detail'})
        ...
        for outbound in outbounds:
            ...
            origin_airport = Airport.objects.get(
                pk=self.originationAirportCode)
            destination_airport = Airport.objects.get(
                pk=self.destinationAirportCode)
            ...
            arrive_date_time = '{} {}'.format(
                arrival_date.strftime('%Y-%m-%d'), arrive_time)
            ...
            f = Flight.objects.create(
                ...
                arrive_time=destination_airport.get_tz_obj().localize(
                    to_datetime(arrive_date_time)),
            ...
            layovers, duration, change_planes = self.get_layover_cities_times(
                outbound)
    ...
    def get_row_stops(self, row_soup):
        stops_raw = row_soup.find(
            'div', {'class': 'select-detail--flight-stops-badge'}).text.strip()
    ...
    def get_layover_cities_times(self, row_soup):
        stops_raw = row_soup.findAll(
            'div', {'class': 'flight-stops--item-text'})
        ...
        for stop_raw in stops_raw:
            ...
            layovers.append(stop_raw.find('button').find(
                'span', {'class': 'actionable--text'}).text.strip())
            duration.append(to_timedelta(stop_raw.find(
                'span', {'class': 'flight-stops--item-description'}).text.strip()))
    ...
    def get_row_prices(self, row_soup):
        ...
        fare_boxes = row_soup.find('div', {
                                   'class': 'select-detail--fares'}).findAll('div', {'class': 'select-detail--fare'})
            ...
            if 'fare-button_disabled' not in fare_box['class']:
                ...
                for fare_type in fares.keys():
                    if label.startswith(fare_type):
                        fares[fare_type] = float(
                            b.find('span', {'class': 'fare-button--value-total'}).text.strip())
        return fares
