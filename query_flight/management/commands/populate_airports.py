...
class Command(BaseCommand):
    ...
    def _get_sw_airports(self,
        ...
        options = soup.find(
            attrs={'class': 'stationInput', 'id': 'originAirport'}).findAll('option')
        ...
        for option in options:
            ...
            if (not option['value'] == '') and (not option.text.strip().startswith('[')):
                ...
                airport_codes['title'].append(
                    option.text.strip().split(' - ')[0])
    ...
    def _add_missing_sw(self, merged_df):
        ...
        def app_row(row):
            if len(Airport.objects.filter(abrev=row['code'])) == 0:
                ...
                print(
                    'Already There {} - {}'.format(row['title'], row['code']))
    ...
    def handle(self, *args, **options):
        ...
        self._add_missing_sw(self._merge_airport_info(self._get_sw_airports(
            proxies=proxies), self._get_all_airports(proxies=proxies)))
